...
pub fn do_test(data: &[u8]) {
    ...
    match (bitreq_result, url_result) {
        (Ok(bitreq_url), Ok(ref_url)) => {
            ...
            let bitreq_path = if bitreq_url.path().is_empty() {
                "/"
            } else {
                bitreq_url.path()
            };
            ...
            let ref_segments: Vec<_> = ref_url
                .path_segments()
            ...
            let ref_pairs: Vec<(String, String)> = ref_url.query_pairs()
                .map(|(k, v)| (k.into_owned(), v.into_owned()))
                .collect();
            let ref_pairs_str: Vec<(&str, &str)> = ref_pairs.iter()
                .map(|(k, v)| (k.as_str(), v.as_str()))
                .collect();
            assert_eq!(
                bitreq_pairs, ref_pairs_str,
                "Query pairs mismatch for input: {:?}",
                input
            );
        }
        (Ok(v), Err(e)) => {
